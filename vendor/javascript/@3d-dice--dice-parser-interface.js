...
class ParserInterface {
  ...
  initParser(){
    this.rollParser = new DiceRoller.DiceRoller((rolls = this.rollsAsFloats) => {
      if(rolls.length > 0) {
  ...
  parseNotation(notation, id) {
    // clean out the gunk
    // this.clear()
    // parse the raw string notation
    notation = notation.replace(/d00/,'d%') // convert d00 to d%
    this.parsedNotation.push(this.rollParser.parse(notation))
    ...
    this.recursiveSearch(this.parsedNotation.at(-1), 'die', [], findDie)
  ...
  parseFinalResults(rollResults = [], id) {
    // do the final parse
    ...
    const finalResults = this.rollParser.rollParsed(this.parsedNotation[id])
    ...
    // Append finalResults
    this.finalResults.push(finalResults)
